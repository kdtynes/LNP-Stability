...
Energy minimized lipid conformations used as input to AP-Net are computed in `models/prepare_dimers/`. SAPT0 energies predicted with AP-Net are stored in `datasets/dimer_interactions_minenergy.pkl` for training and `datasets/dimer_interactions_pfizer_moderna.pkl` for the validation study. AP-Net source code and pretrained models are in `AP-Net-master/`.

## Preprocessing

This script `models/preprocess.py` combines SMILES strings, inferred AP-Net SAPT energies, and calculated Mordred descriptors with the molecular composition and mass ratio, then splits into payload specific datasets for mRNA, siRNA, and combined RNA train/test sets.
...
This script `models/LNP_model.py` trains a final XGBoost model with the best parameters, and scores the held-out test set. Set `model_name` to `'mRNA'`, `'siRNA'`, or `'RNA'` in the main functio. The script loads tuned hyperparameters from `models/params/` to reproduce the models reported in the paper.
